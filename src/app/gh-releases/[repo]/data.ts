...
export const getRepoReleaseData = async (name?: string) => {
  ...
  try {
    const dataRequest = await fetch(
      `https://api.github.com/repos/bjorndonald/${name}/releases/latest`,
      authHeaders,
    );
    ...
    const extraRepoData = {
      success: false,
      url: `https://github.com/bjorndonald/${name}`,
    };
    const defaultDownloadLink = `https://github.com/bjorndonald/${name}/releases/latest/`;
